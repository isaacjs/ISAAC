...
// Distance: gigameters
// Mass: gigagrams
var Gmm = new BigDecimal("7.93e11"); // in Gigagram Gigameters per second squared.
//var Gmm = 
var timeStep = 864; // in seconds.


var Earth = PhysicalObject();
Earth.physical.mass = new BigDecimal("5.9721986e18");
Earth.motion.position = [new BigDecimal("149.598261"), BigDecimal.prototype.ZERO, BigDecimal.prototype.ZERO];
Earth.motion.velocity = [BigDecimal.prototype.ZERO, new BigDecimal("2.929e-5"), BigDecimal.prototype.ZERO];
//Earth.physical.mass = 
...
Sun.motion.position = [BigDecimal.prototype.ZERO, BigDecimal.prototype.ZERO, BigDecimal.prototype.ZERO];
...
function gravitationalForce (earth, sun) {	
	...
	var force = Gmm.divide(distance.pow(new BigDecimal("2")));
...
Sun.motion.position = addVector([new BigDecimal("400"), new BigDecimal("300"), BigDecimal.prototype.ZERO], Sun.motion.position);
Earth.motion.position = addVector([new BigDecimal("400"), new BigDecimal("300"), BigDecimal.prototype.ZERO], Earth.motion.position);
...
function drawCircle (position, radius, ctx) {
	...
	// Draw the arc.
	ctx.arc(parseFloat(position[0].toString()), parseFloat(position[1].toString()), radius, 0, 2*Math.PI, true);
