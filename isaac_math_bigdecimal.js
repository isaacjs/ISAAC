// Math Library for ISAAC Physics, using BigDecimal.

var BigDecimal = window.BigDecimal;
var MathContext = window.MathContext;
...
function addVector (vectorA, vectorB) {
	if(vectorA.length === vectorB.length) {
		...
		for(var i = 0; i < vectorA.length; i++) {
			newVector[i] = vectorA[i].add(vectorB[i]);
		}
		return newVector;
	} else {
		return vectorA;
...
function scaleVector (vector, scalar) {
	...
	for(var i = 0; i < vector.length; i++) {
		newVector[i] = vector[i].multiply(new BigDecimal(scalar.toString()));
	}
	return newVector;
}
...
function dotProduct (vectorA, vectorB) {
	if(vectorA.length === vectorB.length) {
		...
		for (var i = 0; i < vectorA.length; i++) {
			sum.add(vectorA[i].multiply(vectorB[i]));
		}
		return sum;
	} else {
		return -1;
...
function vectorLength (vector) {
	var squares = new BigDecimal("0");

	for (var i = 0; i < vector.length; i++) {
		squares.add(vector[i].pow(2));
	}
	return new BigDecimal((Math.sqrt(parseFloat(squares.toString()))).toString());
}
...
function vectorNormalise (vector) {
	...
	for (var i = 0; i < vector.length; i++) {
		normal[i] = vector[i].divide(new BigDecimal(length.toString()));
	}
	return normal;
}
...
function vectorFitToLength (vector, scalar) {
	...
	for (var i = 0; i < scaled.length; i++) {
		scaled[i] = scaled[i].multiply(new BigDecimal(scalar.toString()));
	}
	return scaled;
}